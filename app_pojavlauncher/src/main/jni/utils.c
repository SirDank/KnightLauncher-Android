...
jobjectArray convert_from_char_array(JNIEnv *env, char **charArray,
                                     int num_rows) {
  jobjectArray resultArr = (*env)->NewObjectArray(
      env, num_rows, (*env)->FindClass(env, "java/lang/String"), NULL);
  jstring row;
...
void free_char_array(JNIEnv *env, jobjectArray jstringArray,
...
Java_net_kdt_pojavlaunch_utils_JREUtils_setupBridgeSurfaceAWT(JNIEnv *env,
                                                              jclass clazz,
                                                              jlong surface) {
...
Java_android_view_Surface_nativeGetBridgeSurfaceAWT(JNIEnv *env, jclass clazz) {
  return (jlong)shared_awt_surface;
}

JNIEXPORT jint JNICALL
Java_android_os_OpenJDKNativeRegister_nativeRegisterNatives(
    JNIEnv *env, jclass clazz, jstring registerSymbol) {
  const char *register_symbol_c =
      (*env)->GetStringUTFChars(env, registerSymbol, 0);
...
JNIEXPORT void JNICALL Java_net_kdt_pojavlaunch_utils_JREUtils_setLdLibraryPath(
    JNIEnv *env, jclass clazz, jstring ldLibraryPath) {
  // jclass exception_cls = (*env)->FindClass(env,
  ...
  if (updateLdLibPath == NULL) {
    updateLdLibPath =
        dlsym(libdl_handle, "__loader_android_update_LD_LIBRARY_PATH");
    if (updateLdLibPath == NULL) {
      char *dl_error_c = dlerror();
      LOGE("Error getting symbol android_update_LD_LIBRARY_PATH: %s",
           dl_error_c);
  ...
  android_update_LD_LIBRARY_PATH((char *)ldLibPathUtf);
  (*env)->ReleaseStringUTFChars(env, ldLibraryPath, ldLibPathUtf);
}

JNIEXPORT jboolean JNICALL Java_net_kdt_pojavlaunch_utils_JREUtils_dlopen(
...
JNIEXPORT jint JNICALL Java_net_kdt_pojavlaunch_utils_JREUtils_chdir(
...
JNIEXPORT jint JNICALL Java_net_kdt_pojavlaunch_utils_JREUtils_executeBinary(
    JNIEnv *env, jclass clazz, jobjectArray cmdArgs) {
...
/*
JNIEXPORT jint JNICALL
Java_net_kdt_pojavlaunch_utils_JREUtils_executeForkedBinary(JNIEnv *env, jclass
clazz, jobjectArray cmdArgs) { int x, status; x = fork(); if (x > 0) {
                wait(&status);
        } else {
                execvpe();
        }
