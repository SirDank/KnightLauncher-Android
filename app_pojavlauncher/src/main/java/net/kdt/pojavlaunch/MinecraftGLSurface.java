...
/**
 * Class dealing with showing minecraft surface and taking inputs to dispatch
 * them to minecraft
 */
public class MinecraftGLSurface extends View implements GrabListener, DirectGamepadEnableHandler {
    ...
    private final double mSensitivityFactor = (1.4
            * (1080f / Tools.getDisplayMetrics((Activity) getContext()).heightPixels));
     ...
     * 
     * @param isAlreadyRunning set to true to tell the view that the game is already
     *                         running
     *                         (only updates the window without calling the start
     *                         listener)
     * @param touchpad         the optional cursor-emulating touchpad, used for
    ...
    /**
     * The touch event for both grabbed an non-grabbed mouse state on the touch
     * screen
    ...
    public boolean onTouchEvent(MotionEvent e) {
        ...
        if (mIngameProcessor == null || mInGUIProcessor == null)
            return true;
    ...
    private void createGamepad(View contextView, InputDevice inputDevice) {
        if (CallbackBridge.sGamepadDirectInput && !sdlEnabled) {
            ...
            }; // Ensure it isn't null while also not processing the events.
    }
    ...
    public boolean dispatchGenericMotionEvent(MotionEvent event) {
        ...
        if (!sdlEnabled && Gamepad.isGamepadEvent(event)) {
            if (mGamepadHandler == null)
                createGamepad(this, event.getDevice());
        ...
        if (mouseCursorIndex == -1)
            return false; // we cant consoom that, theres no mice!
    ...
    public boolean processKeyEvent(KeyEvent event) {
        ...
        if (eventKeycode == KeyEvent.KEYCODE_UNKNOWN)
            return true;
        if (eventKeycode == KeyEvent.KEYCODE_VOLUME_DOWN)
            return false;
        if (eventKeycode == KeyEvent.KEYCODE_VOLUME_UP)
            return false;
        ...
        // In accordance with
        // https://developer.android.com/reference/android/view/KeyEvent#FLAG_CANCELED
        if (action == KeyEvent.ACTION_UP &&
                (event.getFlags() & KeyEvent.FLAG_CANCELED) != 0)
            return true;

        // Sometimes, key events comes from SOME keys of the software keyboard
        // Even weirder, is is unknown why a key or another is selected to trigger a
        // keyEvent
        if ((event.getFlags() & KeyEvent.FLAG_SOFT_KEYBOARD) == KeyEvent.FLAG_SOFT_KEYBOARD) {
            if (eventKeycode == KeyEvent.KEYCODE_ENTER)
                return true; // We already listen to it.
    ...
    private void realStart(Surface surface) {
        // Initial size set. Request immedate refresh, otherwise the initial width and
        // height for the game
        // may be broken/unknown.
        refreshSize(true);
        // Ensures we run at correct refresh rate (should also NOT change the resolution
        // being used)
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S) {
            ...
            surface.setFrameRate(maxHz, Surface.FRAME_RATE_COMPATIBILITY_DEFAULT,
            ...
            try {
                ...
                synchronized (mSurfaceReadyListenerLock) {
                    if (mSurfaceReadyListener == null)
                        mSurfaceReadyListenerLock.wait();
                }
